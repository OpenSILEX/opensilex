<template>
  <div id="v-step-global">
    <ValidationObserver ref="validatorRef">
      ...
      <div class="row">
        <div class="col-lg-6" id="v-step-entity">
          ...
          <opensilex-SelectForm
              ...
              :conversionMethod="objectToSelectNodeEnity"
        ...
        <div class="col-lg-6" id="v-step-characteristic">
          ...
          <opensilex-CharacteristicModalForm
              ...
              @onCreate="setLoadedSeleCharacteristic">
          </opensilex-CharacteristicModalForm>
        </div>
        ...
        <div class="col-lg-6" id="v-step-method">
          <opensilex-SelectForm
              ...
              :disabled="false"
          ></opensilex-SelectForm>
          <opensilex-MethodCreatepageSize
              ref="methodForm"
      ...
      <div class="col">
        <opensilex-LabelCreationSubForm
            :form.sync="form.multiLabelsDTO"
            ref="labelCreationSubForm" @onSubmitSubForm="getConfimedLabelsDTOFromSubForm"
        />
...
import {MultiNamedResourceDTO} from "opensilex-core/model/multiNamedResourceDTO";
...
//        variablesGroup: []
  ...
  searchEntities(label: string, page, pageSize) {
    return this.service.searchEntities(label, ["prefLabel=asc"], page, pageSize)
  ...
  setLoadedEntity(created: EntityCreationDTO) {
    ...
    this.entitySelectForm.select({id: created.uri, label: created.multiLabelsDTO.prefLabels[this.lang]});
  }
  ...
  objectToSelectNode(dto) {
    // console.log("objectToSelectNode",dto);

    if (dto && dto.name) {
      return {id: dto.uri, label: dto.name};
    } else if (dto && dto.multiLabelsDTO.prefLabels) {
      // console.log("objectToSelectNode",dto);
      return {id: dto.uri, label: dto.prefLabels[this.lang]};
    }
    return null;
  }

  objectToSelectNodeEnity(dto) {
    if (dto && dto.prefLabels) {
      return {id: dto.uri, label: dto.prefLabels[this.lang]};
    } else if (dto && dto.multiLabelsDTO.prefLabels) {
      return {id: dto.uri, label: dto.multiLabelsDTO.prefLabels[this.lang]};
    }
    return null;
  }
  ...
  addLabelsToMultiLabelsDTO(labelDTO) {
    console.log("this.entityDto.multiLabelsDTO", JSON.stringify(this.form.multiLabelsDTO));

    if (!this.form.multiLabelsDTO.altLabels) {
      this.form.multiLabelsDTO.altLabels = {};
    }

    this.form.multiLabelsDTO.prefLabels[labelDTO.lang] = labelDTO.prefLabel;

    this.form.multiLabelsDTO.shortLabels[labelDTO.lang] = labelDTO.shortLabel;

    if (!this.form.multiLabelsDTO.altLabels[labelDTO.lang]) {
      this.form.multiLabelsDTO.altLabels[labelDTO.lang] = [];
    }

    for (let i = 0; i < labelDTO.altLabels.length; i++) {
      this.form.multiLabelsDTO.altLabels[labelDTO.lang].push(labelDTO.altLabels[i]);
    }

    this.form.multiLabelsDTO.definitions[labelDTO.lang] = labelDTO.definition;
