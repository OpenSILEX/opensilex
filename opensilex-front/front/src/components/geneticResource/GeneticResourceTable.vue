<template>
  <div class="card">
    <div>
      <opensilex-GeneticResourceAddColumnModal
          class="searchFilter"
          ref="colModal"
    ...
    <b-input-group id="tableOptions" class="mt-2 mb-2" size="sm">
      <downloadCsv
          ...
          name="template.csv"
      >
        {{ $t("GeneticResourceTable.downloadTemplate") }}
      </downloadCsv>
      ...
      >{{ $t("GeneticResourceTable.addColumn") }}
      </b-button
      >
      <b-form-select
    ...
    <b-modal
        ...
        :title="$t('GeneticResourceTable.newColumns')"
    >
      <b-form-group
          :label="$t('GeneticResourceTable.newColumnsHelp')"
      ...
      <b-form-checkbox
          ...
          switch
      >
        {{ $t('GeneticResourceTable.toggleAll') }}
      </b-form-checkbox>
    ...
    <b-modal
        ...
        centered
        hide-footer
        :title="$t('GeneticResourceTable.errorModalTitle')"
    >
      <ul>
...
import {GeneticResourceCreationDTO, GeneticResourceService} from "opensilex-core/index";
...
import GeneticResourceTableDataRow from "./GeneticResourceTableDataRow";
...
@Component
/**
 * The table that's shown during the creation of geneticResources.
 */
export default class GeneticResourceTable extends Vue {
   ...
   * A key is added the first time this column is added (duplicates aloud). The keys are equal to the JsonParams in GeneticResourceCreationDTO, example : hasParentGeneticResourceA
   * The values represent the current id for the tabulator, a digit is added each time.
   * Example hasParentGeneticResourceM0, hasParentGeneticResourceF1, etc...
   */
  ...
  private tableData :Array<GeneticResourceTableDataRow> = [];

  @Watch("tableData", {deep: true})
  newData(value: Array<GeneticResourceTableDataRow>) {
  ...
  private readonly checkBoxOptions = [
    { text: this.$t("GeneticResourceTable.checkBoxOptionsAll"), value: "all" },
    { text: this.$t("GeneticResourceTable.checkBoxOptionsAllErrors"), value: "NOK" },
  ];
  ...
  private onAddRowBtnClick() {
    const newGeneticResource = new GeneticResourceTableDataRow(this.tableData.length);
    this.tableData.push(newGeneticResource,);
    this.tabulator.addRow(newGeneticResource.getData());
  }
  ...
  private onNewColsModalHidden() {
    ...
    this.tableData = this.csvUploadedData.map((row, index) => {
      const geneticResource = new GeneticResourceTableDataRow(index, row);
      return geneticResource;
    });

    this.SetUpdateStatusForEachGeneticResource();
  }
  ...
  /**
   * Looks at the type of geneticResource (species, variety, etc...) to set this.tableColumns.
  ...
  private updateColumns() {
    ...
    let labelCol = {
      title: this.$t("GeneticResourceTable.name") + '<span class="required">*</span>',
      field: "name",
      visible: true,
      editor: true,
      minWidth: 150,
    ...
    let synonymCol = {
      title: this.$t("GeneticResourceTable.synonyms"),
      field: "synonyms",
      visible: true,
      editor: true,
    };
    let subtaxaCol = {
      title: this.$t("GeneticResourceTable.subtaxa"),
      field: "subtaxa",
      visible: true,
      editor: true,
    };
    let speciesCol = {
      title:
          this.$t("GeneticResourceTable.fromSpecies") +
    ...
    let varietyCol = {
      title:
          this.$t("GeneticResourceTable.fromVariety") +
          '<span class="requiredOnCondition">*</span>',
      field: "variety",
      visible: true,
      editor: true,
    ...
    let accessionCol = {
      title:
          this.$t("GeneticResourceTable.fromAccession") +
          '<span class="requiredOnCondition">*</span>',
      field: "accession",
      visible: true,
      editor: true,
    ...
    let instituteCol = {
      title: this.$t("GeneticResourceTable.institute"),
      field: "institute",
      visible: true,
      editor: true,
    };
    let websiteCol = {
      title: this.$t("GeneticResourceTable.website"),
      field: "website",
      visible: true,
      editor: true,
    };
    let productionYearCol = {
      title: this.$t("GeneticResourceTable.year"),
      field: "productionYear",
      visible: true,
      editor: true,
    };
    let commentCol = {
      title: this.$t("GeneticResourceTable.comment"),
      field: "comment",
      visible: true,
      editor: true,
    };
    let isPublicCol = {
      title:this.$t("GeneticResourceTable.is_public"),
      field: "is_public",
      visible: true,
      editor: "select",
      editorParams:{
    ...
    let checkingStatusCol = {
      title: this.$t("GeneticResourceTable.checkingStatus"),
    ...
    let insertionStatusCol = {
      title: this.$t("GeneticResourceTable.insertionStatus"),
    ...
    if (Oeso.checkURIs(this.$attrs.geneticResourceType, Oeso.SPECIES_TYPE_URI)) {
      this.tableColumns = [
        idCol,
        statusCol,
        uriCol,
        ...
        Oeso.checkURIs(this.$attrs.geneticResourceType, Oeso.VARIETY_TYPE_URI)
    ) {
      let codeVar = {
        title: this.$t("GeneticResourceTable.varietyCode"),
        field: "code",
        visible: true,
        editor: true,
      };
      this.tableColumns = [
        ...
        Oeso.checkURIs(this.$attrs.geneticResourceType, Oeso.ACCESSION_TYPE_URI)
    ) {
      let codeAcc = {
        title: this.$t("GeneticResourceTable.accessionNumber"),
        field: "code",
        visible: true,
        editor: true,
      };
      ...
      let codeLot = {
        title: this.$t("GeneticResourceTable.lotNumber"),
        field: "code",
        visible: true,
        editor: true,
      };
    ...
    this.tabulator.on("cellEdited", (cell) => {
      ...
      if (cell.getField() === "uri") {
        ...
        const geneticResource: GeneticResourceTableDataRow = this.tableData[cell.getRow().getData().index];
        if (uri == null || uri === "") {
          geneticResource.setIsUpdate(false);
          return;
        } else {
          this.SetUpdateStatusForEachGeneticResource();
        }
  ...
  private addInitialXRows(X) {
    for (let i = 0; i < X; i++) {
      this.tableData.push(new GeneticResourceTableDataRow(i));
    }
  }
  ...
  private async upsertOrCheckData() {
    ...
    let DTOs: Array<GeneticResourceCreationDTO> = await this.getDtosFromTableData();
  ...
  private async callUpsertService(creationDtos: Array<GeneticResourceCreationDTO>) {
    return this.service.upsertGeneticResources(this.onlyChecking, creationDtos)
        .then( () => {
          let successMessage = this.onlyChecking ? this.$t("GeneticResourceTable.successCheckMessage").toString() : this.$t("GeneticResourceTable.successUpsertMessage").toString();
          this.$opensilex.showSuccessToast(successMessage);

          this.setGeneticResourcesValidationStatus(false);
          this.filter = "all";
        })
        .catch( error => {
          if (error.response.status < 400 || error.response.status >= 500) {
            this.$opensilex.showErrorToast(this.$t("GeneticResourceTable.errorServerMessage").toString());
            return;
          }

          let errorMessage = this.onlyChecking ? this.$t("GeneticResourceTable.errorCheckMessage").toString() : this.$t("GeneticResourceTable.errorUpsertMessage").toString();
          ...
          this.setGeneticResourcesValidationStatus(true);
          errors.forEach(errorDto => {

            let errorMessage = errorDto.errors.length > 1 ?
                this.$t("GeneticResourceTable.multipleErrorMessage", {count: errorDto.errors.length}).toString() :
                errorDto.errors[0];
  ...
  /**
   * set the update status for each geneticResource
   * This will trigger the rowFormatter to change the color of the row.
   */
  private setUpdateStatusForEachGeneticResource(existingUris: string[]){
   ...
   */
  private setGeneticResourcesValidationStatus(upsertWasCancelled: boolean): void {
    this.tableData.forEach((data) => {
      data.setIsValidated();
      data.setInsertionStatus(this.$t(
          upsertWasCancelled ? "GeneticResourceTable.cancelledUpsertRowMessage" : "GeneticResourceTable.successUpsertRowMessage"
      ).toString());
      data.setCheckingStatus(this.$t("GeneticResourceTable.successCheckRowMessage").toString());
    });
  }

  private async getExistingGeneticResourcesUri(): Promise<Array<string>>{
    let uris: URIsListPostDTO = {
      uris: this.tableData.map((row) => {
        return row.getData().uri as string;
      })
    }
    return ( await this.service.checkGeneticResourcesExist(uris) ).response.result
  }

  /**
   * After an API call, this function sets the update status for each geneticResource (true or false).
   */
  private SetUpdateStatusForEachGeneticResource(){
    this.getExistingGeneticResourcesUri()
        .then(uris => {
          this.setUpdateStatusForEachGeneticResource(uris);
        });
  }

  private async getDtosFromTableData(): Promise<Array<GeneticResourceCreationDTO>> {
    let creationDtos: Array<GeneticResourceCreationDTO> = [];
    ...
    for (let idx = 0; idx < dataToInsert.length; idx++) {
      ...
      form.rdf_type = this.$attrs.geneticResourceType;
  ...
  /**
   * Called when we upload a csv. Verifies that each geneticResource has a unique uri.
   * Initialises this.newColumns to all from the csv that were not already present in this.tableColumns.
   */
  private uploaded(data) {
    if (data.length > 1000) {
      alert(this.$t("GeneticResourceTable.alertFileSize"));
    } else {
      this.newColumns = [];
      let uniqueURIs = [];
      ...
      for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
        ...
        if (currentName === "") {
          alert(
              this.$t("GeneticResourceTable.missingName") + " " + rowIndex
          );
          insertionOK = false;
          break;
        }
        if (currentUri !== "") {
          if (uniqueURIs.indexOf(currentUri) === -1) {
            ...
            insertionOK = false;
            alert(
                this.$t("GeneticResourceTable.alertDuplicateURI") +
                " " +
                rowIndex +
                ", uri= " +
                currentUri
      ...
      if (insertionOK) {
        ...
        if (this.newColumns.length > 0) {
          ...
          this.tableData = dataInJsonFormat.map( (row, index) =>  new GeneticResourceTableDataRow(index, row));
        }
      }
    }
    this.SetUpdateStatusForEachGeneticResource();
...
en:
  GeneticResourceTable:
    ...
    successCheckMessage: Genetic Resources are ready to be inserted and/or updated
    ...
    successUpsertMessage: Genetic Resources inserted and/or updated
    successUpsertRowMessage : inserted/updated
    cancelledUpsertRowMessage : insertion/update cancelled
    errorModalTitle: Genetic Resource contain following errors
    multipleErrorMessage: click here to see {count} errors for this genetic resource
    addRow: Add Row
