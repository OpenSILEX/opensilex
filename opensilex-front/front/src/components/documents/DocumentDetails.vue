            ...
            <template v-slot:body>
              ...
              <opensilex-StringView class="overflow-auto" style="height: 100px" label="DocumentDetails.targets" :uri="document.targets">
                ...
                <opensilex-UriLink 
                  :uri="target.uri"  
                  :value="target.uri"  
                  v-else-if="target.rdf_types.includes($opensilex.Oeso.GENETIC_RESOURCE_TYPE_URI)"           
                  :to="{path: '/geneticResource/details/'+ encodeURIComponent(target.uri)}"
                ></opensilex-UriLink>
                <opensilex-UriLink 
                  :uri="target.uri" 
...
export default class DocumentDetails extends Vue {
  ...
  loadTargetsTypes() {
    ...
    let types = new Array(Oeso.GENETIC_RESOURCE_TYPE_URI, Oeso.DEVICE_TYPE_URI, Oeso.PROJECT_TYPE_URI, Oeso.EXPERIMENT_TYPE_URI, Oeso.VARIABLESGROUP_TYPE_URI);
    let body = {
