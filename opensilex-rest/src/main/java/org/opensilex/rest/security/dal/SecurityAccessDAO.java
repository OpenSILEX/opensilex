...
public class SecurityAccessDAO {
    ...
    private void addOperationAccess(String pathUri, Operation operation, HttpMethod method, Map<String, Map<String, String>> accessMap) {
        if (operation != null) {
            ...
            Object extensionMap = operation.getVendorExtensions().get(ApiProtected.OPENSILEX_CREDENTIAL_EXTENSION);

            boolean ignoreCredential = false;
            if (extensionMap != null && ClassUtils.isMap(extensionMap.getClass())) {
                Map extension = (Map) extensionMap;
                String extensionValue = (String) extension.getOrDefault(ApiProtected.OPENSILEX_NO_CREDENTIAL_REQUIRED_PROPERTY, ApiProtected.OPENSILEX_NO_CREDENTIAL_REQUIRED_PROPERTY_VALUE);
                ignoreCredential = extensionValue.equals(ApiProtected.OPENSILEX_NO_CREDENTIAL_REQUIRED_PROPERTY_VALUE);
            }
