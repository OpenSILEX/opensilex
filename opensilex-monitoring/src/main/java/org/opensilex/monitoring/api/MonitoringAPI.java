...
import org.opensilex.monitoring.api.dto.GraphTripleCountDTO;
import org.opensilex.monitoring.api.dto.HealthCheckDTO;
import org.opensilex.monitoring.api.dto.MongoCollectionStatsDTO;
...
public class MonitoringAPI {
    ...
    public Response dbStats() {
        ...
        dto.setTripleStore(toDto(stats.tripleStoreVolumetry(), stats.isTripleStoreVolumetryCached()));
        dto.setMongodb(toDto(stats.mongoVolumetry(), stats.isMongoVolumetryCached()));
    ...
    private static TripleStoreStatsDTO toDto(TripleStoreVolumetry volumetry, boolean cached) {
        ...
        dto.setComputedAt(volumetry.computedAt());
        dto.setMessage(volumetry.message());
        dto.setCached(cached);

        List<GraphTripleCountDTO> graphs = new ArrayList<>();
        volumetry.graphs().forEach(graph -> {
            GraphTripleCountDTO graphDto = new GraphTripleCountDTO();
            graphDto.setGraph(graph.graph());
            graphDto.setTripleCount(graph.tripleCount());
            graphs.add(graphDto);
        });
        dto.setGraphs(graphs);
        return dto;
    }

    private static MongoStatsDTO toDto(MongoVolumetry volumetry, boolean cached) {
        MongoStatsDTO dto = new MongoStatsDTO();
        dto.setDatabase(volumetry.database());
        dto.setObjectCount(volumetry.objectCount());
        dto.setCollectionCount(volumetry.collectionCount());
        dto.setDataSizeBytes(volumetry.dataSizeBytes());
        dto.setStorageSizeBytes(volumetry.storageSizeBytes());
        dto.setIndexSizeBytes(volumetry.indexSizeBytes());
        dto.setComputedAt(volumetry.computedAt());
        dto.setMessage(volumetry.message());
        dto.setCached(cached);

        List<MongoCollectionStatsDTO> collections = new ArrayList<>();
        volumetry.collections().forEach(collection -> {
            MongoCollectionStatsDTO collectionDto = new MongoCollectionStatsDTO();
            collectionDto.setName(collection.name());
            collectionDto.setDocumentCount(collection.documentCount());
            collectionDto.setSizeBytes(collection.sizeBytes());
            collectionDto.setStorageSizeBytes(collection.storageSizeBytes());
            collectionDto.setIndexSizeBytes(collection.indexSizeBytes());
