...
import phis2ws.service.view.brapi.results.ResultUri;
import phis2ws.service.view.manager.ResultForm;
import phis2ws.service.view.model.phis.Uri;

/**
 * Allows the formating of the result of the request about URIs
 * @author Eloan LAGIER
 */
public class ResponseFormUri extends ResultForm<Uri>{
    ...
    public ResponseFormUri(Integer pageSize, Integer currentPage, ArrayList<Uri> list, boolean paginate) {
        ...
        if (list.size() > 1) {
            result = new ResultUri(list, metadata.getPagination(), paginate);
        } else {
           result = new ResultUri(list);
        }
     }
}
