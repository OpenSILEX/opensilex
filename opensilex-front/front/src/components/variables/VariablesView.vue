<template>
  <div class="container-fluid">
    <div>
      ...
      <div class="col-lg-5">
        ...
        >
          ...
          <b-modal ref="helpModal" size="xl" hide-header hide-footer>
            <opensilex-VariableHelp v-if="elementType != 'VariableGroup'"
                                    @hideBtnIsClicked="hide()"></opensilex-VariableHelp>
      ...
      <div class="row">
        <div class="col-md-6" v-if="! loadVariableList() ">
          <!-- Element list -->
          <opensilex-EntityStructureList
              v-if="elementType == 'Entity'"
              ref="EntityStructureList"
        ...
        <div class="col-md-6">
          ...
          <opensilex-VariableStructureDetails
              v-else
              v-show="!loadVariableList() && useGenericDetailsPage()"
              :selected="selected"></opensilex-VariableStructureDetails>
          <!-- Unit specialized details page -->
          <opensilex-UnitDetails v-show="!loadVariableList() && !useGenericDetailsPage() && !groupVariablesPage()"
                                 :selected="selected"></opensilex-UnitDetails>

          <opensilex-Card
              v-show="!groupVariablesPage() && !loadVariableList()" label="component.skos.ontologies-references-label"
          ...
          <opensilex-Card
              v-if="!loadVariableList() && useGenericDetailsPage() && groupVariablesPage() && selected.variables != undefined"
              :label="$t('VariableView.type')" :selected="selected" icon="fa#vials">
            <template v-slot:body>
...
import EntityStructureList from "./views/EntityStructureList.vue";
...
export default class VariablesView extends Vue {
  ...
  @Ref("EntityStructureList") readonly EntityStructureList!: EntityStructureList;
  ...
  selectedEntity: EntityDetailsDTO = {
    uri: null,
    multiLabelsDTO: {
      prefLabels: {},
      altLabels: {},
  ...
  updateReferences(variable) {
    ...
    this.service.updateVariable(formattedVariable).then(() => {
      let message = this.$i18n.t("VariableView.name") + " " + formattedVariable.multiLabelsDTO.prefLabels[this.$i18n.locale] + " " + this.$i18n.t("component.common.success.update-success-message");
