import { onMounted, onUnmounted, shallowRef, watch, type Ref, type ShallowRef } from 'vue';
...
export interface EChartsHandle {
    chart: ShallowRef<EChartsInstance | null>;
    setOption: (option: EChartsCoreOption) => void;
    resize: () => void;
    toDataURL: () => string | undefined;
}

export function useECharts(container: Ref<HTMLElement | null>): EChartsHandle {
    // shallowRef, not ref: an ECharts instance owns its own internal reactivity-hostile state
    // (a canvas, private fields), and ref()'s deep UnwrapRef walk over it is both wasted work and,
    // as it turns out, exactly what strips the class's nominal type when the value is read back.
