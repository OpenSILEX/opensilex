<template>
  <Modal ref="modalRef">
    <template #header>
      <FormHeader
        :title="modalFormLogic.formTitle.value"
        icon="bi#bi-activity"
      />
    </template>
        ...
        <div class="col">
          ...
          >
            <TypeForm
              ...
              :placeholder="t('component.events.type-placeholder')"
        ...
        >
          ...
          >
            <TagInputForm
              ...
              :label="t('component.events.targets')"
              type="text"
              :required="true"
        ...
        >
          <n-form-item path="description">
            <TextAreaForm
              ...
              :helpMessage="t('component.events.description')"
              :placeholder="t('component.events.description')"
              @keydown.enter.stop
            />
          </n-form-item>
        </div>
    ...
    <template #footer>
      <FormFooter
        @cancel="modalFormLogic.hide"
        @submit="modalFormLogic.submit"
      />
    </template>
  </Modal>
</template>
...
watch(
  () => modalFormLogic.form.value.targets,
  () => {
...
async function create(formData: MoveCreationDTO) {
  ...
  return isMoveEvent ? service.createMoves(events) : service.createEvents(events);
}
...
function showEditForm(uri: string, type: string, sourceItem?: any) {
  const detailsPromise = isMove(type) ? service.getMoveEvent(uri) : service.getEventDetails(uri);
