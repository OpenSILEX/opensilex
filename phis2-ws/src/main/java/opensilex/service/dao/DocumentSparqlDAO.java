...
public class DocumentSparqlDAO extends SparqlDAO<Document> {
    ...
    /**
     * Prepares the query to search the elements which are concerned by the document.
     * @param uriDocument
    ...
    private SPARQLQueryBuilder prepareSearchConcernedItemsQuery(String uriDocument) {
     ...
     /**
     * Prepares the count query generated with the searched parameters.
     * @example
    ...
    /**
     * Checks if the given user has the right to see the document.
     * @param user
     * @param document
     * @return true if the user can see the document
     *         false if not
     */
    private boolean canUserSeeDocument(User user, Document document) {
        UserDAO userDao = new UserDAO();
        userDao.isAdmin(user);
        if (user.getAdmin().equals("t") || user.getAdmin().equals("true")) {
            return true;
        } else {
            ...
            for (ConcernedItemDTO concernedItem : document.getConcernedItems()) {
                if (concernedItem.getTypeURI().equals(Oeso.CONCEPT_EXPERIMENT.toString())) {
                    ...
                    if (experimentDao.canUserSeeExperiment(user, experiment)) {
                        return true;
                    }
                } 
            }
    ...
    /**
     * Retrieves the list of the searched documents.
    ...
    //\SILEX:todo
    /**
     * Checks the metadata and updates it if the metadata is valid.
    ...
    /**
     * Checks the new metadata and updates it if no error.
     * @param objectsToUpdate
     * @return the update result, with errors or information
     */
