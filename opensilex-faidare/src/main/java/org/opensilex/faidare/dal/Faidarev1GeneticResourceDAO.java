/*
 * *****************************************************************************
 *                         Faidarev1GeneticResourceDAO.java
 ...
 */
public class Faidarev1GeneticResourceDAO extends GeneticResourceDAO {
    public Faidarev1GeneticResourceDAO(SPARQLService sparql, MongoDBServiceV2 nosql) {
        super(sparql, nosql);
    }

    //@todo: remove nosql parameter when experimentDAO can be instanciated with MongoDBServiceV2
    public ListWithPagination<Faidarev1GeneticResourceModel> faidareSearch(AccountModel user, URI geneticResourceDbId, String geneticResourceName, int page, int pageSize, MongoDBService nosql) throws Exception {
        ...
        WHERE
          { GRAPH <http://phenome.inrae.fr/diaphen/set/geneticResource>
              { ?uri  rdf:type  vocabulary:Accession
                OPTIONAL
                ...
                OPTIONAL
                  { GRAPH ?experiment_uri
                      { ?so  vocabulary:hasGeneticResource  ?uri}}
                OPTIONAL
                  { GRAPH <http://phenome.inrae.fr/diaphen/set/geneticResource>
        ...
        */
        ...
        final Node geneticResourceGraph = sparql.getDefaultGraph(GeneticResourceModel.class);
        ...
        WhereBuilder experimentsWhere = new WhereBuilder().addGraph(experimentUriVar, soVar, Oeso.hasGeneticResource.asNode(), uriVar);
        SPARQLQueryHelper.addWhereUriValues(experimentsWhere, experimentUriVar.getVarName(), userExperiments);

        accessions.addGraph(geneticResourceGraph, new WhereBuilder()
                ...
                .addOptional(
                        new WhereBuilder().addGraph(geneticResourceGraph, uriVar, RDFS.label.asNode(), labelVar)
                )
        );
        ...
        if (Objects.nonNull(geneticResourceDbId)) {
            SPARQLQueryHelper.addWhereUriValues(accessions, uriVar.getVarName(), Collections.singletonList(geneticResourceDbId));
        }

        if (Objects.nonNull(geneticResourceName)) {
            SPARQLQueryHelper.addWhereValues(accessions, labelVar.getVarName(), Collections.singletonList(geneticResourceName));
        }

        accessions.setLimit(pageSize);
        accessions.setOffset(page*pageSize);

        List<Faidarev1GeneticResourceModel> searchResults =  sparql.executeSelectQueryAsStream(accessions).map(
                row -> {
                    try {
                        Faidarev1GeneticResourceModel model = new Faidarev1GeneticResourceModel()
