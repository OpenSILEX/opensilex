...
@Component
export default class LogHistogram extends Vue {
  ...
  private initChart() {
    ...
    const option = {
      title: {
        text: "Log-Scaled sample density per variable",
        left: "center",
        textStyle: {
          color: "#333",
          fontSize: 18,
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `${params.seriesName} <br/>${params.name}: ${dataItem.value}<br/>Frequency: ${dataItem.occurrence}`;
        },
      },
      legend: {
        orient: "horizontal",
