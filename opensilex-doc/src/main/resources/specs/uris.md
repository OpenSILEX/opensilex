...
The prefix mapping is stored in a `PrefixMapping` object in `SPARQLService` and `URIDeserializer` (the two references
the same object). It is constructed using static methods, like `SPARQLService#addPrefix`.
