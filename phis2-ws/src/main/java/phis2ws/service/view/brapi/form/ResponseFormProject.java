...
import phis2ws.service.view.brapi.results.ResultatProject;
...
public class ResponseFormProject extends ResultForm<Project> {
    ...
    public ResponseFormProject(int pageSize, int currentPage, ArrayList<Project> projects, boolean paginate) {
        ...
        if (projects.size() > 1) {
            result = new ResultatProject(projects, metadata.getPagination(), paginate);
        } else {
            result = new ResultatProject(projects);
        }
    }
}
