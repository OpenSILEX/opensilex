      ...
      <b-table-simple responsive>
        <b-thead>
          <b-tr>
            ...
            <b-th>start_date_of_Location</b-th>
            <b-th>end_date_of_Location</b-th>
            <b-th>{{ $t("component.common.geometry.from") }}</b-th>
            <b-th>{{ $t("component.common.geometry.too") }}</b-th>
            <b-th>{{ $t("component.common.geometry.coordinates") }}</b-th>
            <b-th>{{ $t("component.common.geometry.x") }}</b-th>
            <b-th>{{ $t("component.common.geometry.y") }}</b-th>
            <b-th>{{ $t("component.common.geometry.z") }}</b-th>
            <b-th>{{ $t("component.common.geometry.textual-position") }}</b-th>
          </b-tr>
        </b-thead>
        <b-tbody>
          <b-tr>
            ...
            <b-td>{{ $t("component.scientificObjects.geometry.startHelp") }}</b-td>
            <b-td>{{ $t("component.scientificObjects.geometry.endHelp") }}</b-td>
            ...
            <b-td>{{ $t("component.common.geometry.textual-position-help") }}</b-td>
          </b-tr>
...
en :
  ScientificObjectImportHelp:
    ...
    text-help:  "Your can insert or update your data from this row. <br /> \n
            ...
            It is now possible to import an initial position by using the new columns.</strong><br>
            You can also update this initial position completely if it is the only existing Position, or partially with dates that match up if multiple positions exist.<br>
            If the location headers are present, but all the location values are empty for a line, then the system considers it to be a deletion of Location.<br>
            If the location headers and columns are completely absent, then no operations will be performed on Locations."
fr :
  ScientificObjectImportHelp:
    ...
    text-help:  "Vous pouvez insérer ou mettre à jour vos données à partir de cette ligne. <br /> \n
            ...
            Vous avez maintenant la posibilité d'importer une position initiale en utilisant les nouvelles colonnes.</strong><br>
            Vous avez aussi la possibilité de mettre à jour cette position initiale, entièrement s'il n'y a qu'une seule position existante, ou partiellement avec des dates qui collent dans le cas où de multiples positions existent.<br>
            Si les en-têtes de géospatialisantion sont présentes dans le CSV, mais que toutes les valeurs de géospatialisation sont vides pour une ligne, alors le système considère que c'est une suppression de position.<br>
            Si les en-têtes et colonnes de géospatialisantion sont complétement absentes, alors aucune opération sera réaliser sur la géospatialisation."
</i18n>