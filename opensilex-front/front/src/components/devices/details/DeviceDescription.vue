...
export default class DeviceDescription extends Vue {
  ...
  loaded_person_in_charge : PersonDTO = null
  ...
  loadPersonInCharge() {
    if (this.device.person_in_charge) {
      this.$opensilex.getService<SecurityService>("opensilex.SecurityService")
          .getPerson(this.device.person_in_charge)
          .then((http: HttpResponse<OpenSilexResponse<PersonDTO>>) => {
            this.loaded_person_in_charge = http.response.result
          })
          .catch(this.$opensilex.errorHandler);
    }
  }
